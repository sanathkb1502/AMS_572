...
-   **Scenario**: This scenario used will be a replication of a recession or pandemic where students have a increased dropout rate in their second semester in relation to a negative change in GDP. The second semester has been chosen due to a lack of data in later semesters but also allows for a previous data point for imputation. When a student drops out their second semester grade will become NA. The values choosen are 20% of dropouts will have a null when the GDP is positive in the 2nd semester grades and when GDP is negative this rate will be 50%. This creates 462 nulls.
...
The initial model used the first semester grade as the predictor instead of second semester due to co-linearity. This model will use the second semester to fit into the scenario. While this may change some p values, the results should not drastically change.
...
We will attempt ot use MICE to reimput the missing nulls.
```{r}
